#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Train/calibrate Gemma4 MC-SMoE checkpoints with comparable recovery strategies."""
...
SIDECAR_FILES = (
    "configuration_gemma4_moe_compressed.py",
    "modeling_gemma4_moe_compressed.py",
    "expert_mapping.json",
    "expert_id_mappings.pt",
    "group_state_dict.pt",
    "chat_template.jinja",
)
...
def _differentiable_compressed_experts_forward(
    self,
    ...
    with torch.no_grad():
        expert_hit = torch.greater(weights_by_physical.detach().sum(dim=0), 0).nonzero(as_tuple=False).flatten()
...
def enable_differentiable_router_weights(model: nn.Module) -> int:
    patched = 0
    for module in model.modules():
        if not hasattr(module, "expert_id_mapping"):
            continue
        if not hasattr(module, "gate_up_proj") or not hasattr(module, "down_proj"):
            continue
        if getattr(module, "_mcsmoe_differentiable_forward", False):
            continue
        module.forward = MethodType(_differentiable_compressed_experts_forward, module)
        module._mcsmoe_differentiable_forward = True
        patched += 1
    if patched:
        print(f"[Patch] Enabled differentiable routing weights for {patched} compressed expert module(s).", flush=True)
    return patched
...
def load_gemma4_model(path: str, args: argparse.Namespace, trainable: bool) -> nn.Module:
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to load Gemma4/MC-SMoE model. Make sure a compatible Gemma4 "
    ...
    if trainable:
        enable_differentiable_router_weights(model)
...
def enable_selected_expert_training(
    ...
    if not hasattr(model, "_mcsmoe_gradient_hooks"):
        model._mcsmoe_gradient_hooks = []
    ...
    for prefix, selected_ids in selected_experts_by_prefix.items():
        ...
        for param_name in ("gate_up_proj", "down_proj"):
            ...
            if param is None:
                raise RuntimeError(f"Packed expert tensor missing: {prefix}.experts.{param_name}")
            param.requires_grad = True
            model._mcsmoe_gradient_hooks.append(param.register_hook(make_expert_mask_hook(selected_ids)))
...
def project_teacher_router_logits_to_student_space(
    ...
    mapping = get_logical_to_physical(student_layer, teacher_logits.device)
    if mapping.numel() != teacher_dim:
        raise ValueError(
            "Cannot map teacher router logits to student router space: "
            f"teacher_dim={teacher_dim}, mapping_len={mapping.numel()}, student_dim={student_dim}"
        )

    physical_num_experts = int(
        getattr(student_layer.experts, "physical_num_experts", getattr(student_layer.experts, "num_experts", 0))
    )
    if student_dim != physical_num_experts:
        raise ValueError(
            "Teacher/student router dimensions differ, but student router dimension does not match "
            f"physical_num_experts: teacher_dim={teacher_dim}, student_dim={student_dim}, "
            f"physical_num_experts={physical_num_experts}"
        )

    flat_teacher = teacher_logits.reshape(-1, teacher_dim).float()
    projected = flat_teacher.new_full((flat_teacher.shape[0], student_dim), float("-inf"))
    for physical_id in range(student_dim):
        logical_mask = mapping == physical_id
        if logical_mask.any():
            projected[:, physical_id] = torch.logsumexp(flat_teacher[:, logical_mask], dim=-1)
        else:
            projected[:, physical_id] = 0.0
    return projected.reshape(*teacher_logits.shape[:-1], student_dim).to(dtype=teacher_logits.dtype)
...
def router_logit_matching_loss(
    ...
    for teacher_prefix, student_prefix in router_prefix_pairs:
        ...
        student_layer = student_module_dict[student_prefix]
        teacher_target = project_teacher_router_logits_to_student_space(
...
def save_mcsmoe_sidecars(source_dir: str, output_dir: str) -> None:
...
def save_model(
    ...
    save_mcsmoe_sidecars(source_checkpoint_path(args), args.output_dir)
    ...
    summary = {
        ...
        "full_model_training": args.strategy in FULL_MODEL_STRATEGIES,
        ...
        "resolved_experts_per_layer": EXPERT_SUBSET_SIZE.get(args.strategy),
        "selection_top_k": args.selection_top_k,
        "router_logit_matching_mapping_policy": "direct_when_dims_match_else_logsumexp_original_to_physical",
        "seed": args.seed,
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.strategy == "direct_router_logit_matching":
        ...
        train_direct_router_logit_matching(args, tokenizer, student, teacher, trainable_params, dataset)
        del teacher
        gc.collect()
        if torch.cuda.is_available():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gemma4 MC-SMoE recovery strategy trainer.")
