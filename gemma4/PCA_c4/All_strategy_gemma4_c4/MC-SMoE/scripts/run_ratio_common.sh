...
wait_stage router_top16_expert_kd router_top50_expert_kd router_top8_expert_kd
  ...
  if ! CUDA_VISIBLE_DEVICES="${VALIDATION_GPU:-0}" ASSIGNED_GPU_IDS="${VALIDATION_GPU:-0}" python "${ROOT_DIR}/validate_saved_models.py"     --base_dir "$ROOT_DIR"     --ratio "$ratio"     --expected_physical_experts "$expected_physical"     >"${ROOT_DIR}/logs/${ratio}/validation.stdout.log" 2>&1; then
