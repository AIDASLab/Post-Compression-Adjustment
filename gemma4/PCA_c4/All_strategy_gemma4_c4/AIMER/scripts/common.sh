...
GEMMA4_ROOT="$(cd "${ROOT_DIR}/../../.." && pwd)"
CONDA_ROOT="${CONDA_ROOT:-}"
CONDA_ENV_NAME="${CONDA_ENV_NAME:-}"
AIMER_COMPRESSED_ROOT="${AIMER_COMPRESSED_ROOT:-${GEMMA4_ROOT}/compression_gemma4/AIMER}"

TEACHER_MODEL="${TEACHER_MODEL:-google/gemma-4-26B-A4B-it}"
DATASET_PATH="${DATASET_PATH:-${C4_DATASET_PATH:-}}"
...
MAX_CALIB_SAMPLES="${MAX_CALIB_SAMPLES:-3000}"
...
EXPERT_SELECTION_TOP_K="${EXPERT_SELECTION_TOP_K:-8}"
...
student_model_for_ratio() {
  case "$1" in
    ratio_50)
      echo "${AIMER_COMPRESSED_ROOT}/gemma4_aimer_keep50"
      ;;
    ratio_625)
      echo "${AIMER_COMPRESSED_ROOT}/gemma4_aimer_keep62_5"
      ;;
    ratio_75)
      echo "${AIMER_COMPRESSED_ROOT}/gemma4_aimer_keep75"
      ;;
    *)
...
gpus_for_strategy() {
  case "$1" in
    full_finetune) echo "${GPUS_FULL_FINETUNE:-4,5}" ;;
    router_top128_expert_finetune) echo "${GPUS_TOP128_FINETUNE:-6,7}" ;;
    only_router_finetune) echo "${GPUS_ROUTER_FINETUNE:-0,1}" ;;
    router_top8_expert_finetune) echo "${GPUS_TOP8_FINETUNE:-2,3}" ;;
    router_top16_expert_finetune) echo "${GPUS_TOP16_FINETUNE:-4,5}" ;;
    router_top50_expert_finetune) echo "${GPUS_TOP50_FINETUNE:-6,7}" ;;
    only_router_kd) echo "${GPUS_ROUTER_KD:-0,1}" ;;
    direct_router_logit_matching) echo "${GPUS_DRLM:-2,3}" ;;
    router_top8_expert_kd) echo "${GPUS_TOP8_KD:-4,5}" ;;
    router_top16_expert_kd) echo "${GPUS_TOP16_KD:-0,1}" ;;
    router_top50_expert_kd) echo "${GPUS_TOP50_KD:-2,3}" ;;
    router_top128_expert_kd) echo "${GPUS_TOP128_KD:-4,5}" ;;
    full_kd) echo "${GPUS_FULL_KD:-6,7}" ;;
    *)
...
python_common_flags() {
  ...
  if [[ -n "$MAX_TRAIN_STEPS" ]]; then
    flags+=(--max_train_steps "$MAX_TRAIN_STEPS")
  fi
  if [[ -n "$ATTN_IMPLEMENTATION" ]]; then
    flags+=(--attn_implementation "$ATTN_IMPLEMENTATION")
  fi
  printf '%s
' "${flags[@]}"
}
