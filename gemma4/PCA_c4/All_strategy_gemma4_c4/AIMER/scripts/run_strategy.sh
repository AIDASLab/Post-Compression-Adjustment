...
if [[ -z "${DATASET_PATH}" || ! -f "${DATASET_PATH}" ]]; then
  echo "Set DATASET_PATH (or C4_DATASET_PATH) to the downloaded C4 JSONL shard." >&2
  exit 2
fi
...
case "$STRATEGY" in
  direct_router_logit_matching|only_router_kd|router_top8_expert_kd|router_top16_expert_kd|router_top50_expert_kd|router_top128_expert_kd|full_kd)
...
case "$STRATEGY" in
  router_top8_expert_kd|router_top8_expert_finetune)
    ARGS+=(--experts_per_layer 8 --selection_logging_steps 20)
    ;;
  router_top16_expert_kd|router_top16_expert_finetune)
    ARGS+=(--experts_per_layer 16 --selection_logging_steps 20)
    ;;
  router_top50_expert_kd|router_top50_expert_finetune)
    ARGS+=(--experts_per_layer 50 --selection_logging_steps 20)
    ;;
esac

case "$STRATEGY" in
  router_top8_expert_kd|router_top8_expert_finetune|router_top16_expert_kd|router_top16_expert_finetune|router_top50_expert_kd|router_top50_expert_finetune)
    if [[ -n "$EXPERT_SELECTION_TOP_K" ]]; then
      ARGS+=(--selection_top_k "$EXPERT_SELECTION_TOP_K")
    fi
    ;;
esac
