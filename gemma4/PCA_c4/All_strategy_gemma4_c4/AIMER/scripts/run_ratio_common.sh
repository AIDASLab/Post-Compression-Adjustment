...
run_ratio_pipeline() {
  local ratio="$1"
  local expected_retained
  local expected_pruned
  expected_retained="$(expected_retained_for_ratio "$ratio")"
  expected_pruned="$(expected_pruned_for_ratio "$ratio")"
...
wait_stage router_top16_expert_kd router_top50_expert_kd router_top8_expert_kd
  ...
  if ! CUDA_VISIBLE_DEVICES="${VALIDATION_GPU:-0}" ASSIGNED_GPU_IDS="${VALIDATION_GPU:-0}" python "${ROOT_DIR}/validate_saved_models.py"     --base_dir "$ROOT_DIR"     --ratio "$ratio"     --expected_retained_experts "$expected_retained"     --expected_pruned_experts "$expected_pruned"     >"${ROOT_DIR}/logs/${ratio}/validation.stdout.log" 2>&1; then
  ...
  if ! python "${ROOT_DIR}/analyze_convergence.py"     --base_dir "$ROOT_DIR"     --ratio "$ratio"     >"${ROOT_DIR}/logs/${ratio}/convergence.stdout.log" 2>&1; then
  ...
  if ! python "${ROOT_DIR}/summarize_gpu_usage.py"     --base_dir "$ROOT_DIR"     >"${ROOT_DIR}/logs/${ratio}/gpu_summary.stdout.log" 2>&1; then
