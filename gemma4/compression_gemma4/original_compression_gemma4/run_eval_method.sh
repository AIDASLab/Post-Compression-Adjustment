...
VALID_METHODS=("original" "AIMER" "MC-SMoE" "all")
...
Examples:
  bash run_eval_method.sh original MC-SMoE AIMER
  bash run_eval_method.sh original MC-SMoE --categories MC,AIME
...
for method in "${METHODS[@]}"; do
  if [[ "${method}" == "all" ]]; then
    EXPANDED_METHODS+=("original" "AIMER" "MC-SMoE")
  else
...
launch_job() {
  ...
  (
    ...
    if [[ "${method}" == "MC-SMoE" ]]; then
      export ORIGCOMP_PATCH_VLLM_GEMMA4="1"
      ...
      unset ORIGCOMP_PATCH_VLLM_GEMMA4
    fi
    ...
    echo "VLLM_RPC_BASE_PATH=${VLLM_RPC_BASE_PATH}"
    echo "VLLM_ENGINE_READY_TIMEOUT_S=${VLLM_ENGINE_READY_TIMEOUT_S}"
    echo "ORIGCOMP_PATCH_VLLM_GEMMA4=${ORIGCOMP_PATCH_VLLM_GEMMA4:-}"
    "${PY_CMD[@]}" "${SCRIPT_PATH}" \
      --method "${method}" \
      --ratio "${ratio}" \
      --variant "${variant}" \
