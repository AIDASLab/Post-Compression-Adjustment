...
def apply() -> None:
    ...
    if getattr(tf_base.Base, "_gemma4_strategy_mixed_attention_patch", False):
        return
        ...
        for layer_idx in range(start, end):
            ...
            if is_sliding:
                ...
                if getattr(text_config, "attention_k_eq_v", False):
                    total_global_kv = int(
                        getattr(text_config, "num_global_key_value_heads", layer_num_kv_heads)
                    )
    ...
    tf_base.Base._gemma4_strategy_mixed_attention_patch = True
