#!/usr/bin/env python3
"""Create an lm-harness_results_gemma4_math-local model mirror for Gemma4 eval.
...
ADDITIONAL_STRATEGY_ROOT = EXP_ROOT.parent
MIRROR_ROOT = EXP_ROOT / "model_mirrors"
GEMMA4_ROOT = Path(__file__).resolve().parents[4]
INPUT_ROOTS = {
    "AIMER_math": ADDITIONAL_STRATEGY_ROOT / "AIMER_math",
    "MC-SMoE_math": ADDITIONAL_STRATEGY_ROOT / "MC-SMoE_math",
}
PROCESSOR_TEMPLATE_CANDIDATES = (
    GEMMA4_ROOT / "PCA_c4" / "All_strategy_gemma4_c4" / "AIMER" / "ratio_50" / "full_finetune" / "processor_config.json",
    GEMMA4_ROOT / "compression_gemma4" / "AIMER" / "gemma4_aimer_keep50" / "processor_config.json",
)
...
def patch_mcsmoe_modeling_file(path: Path) -> None:
    ...
    if class_anchor not in text:
        raise RuntimeError(f"failed to patch MC-SMoE_math modeling copy: class anchor not found in {path}")
    ...
    if attn_anchor not in text:
        raise RuntimeError(f"failed to patch MC-SMoE_math modeling copy: attention anchor not found in {path}")
...
def patch_mcsmoe_config_file(path: Path) -> None:
    ...
    if anchor not in text:
        raise RuntimeError(f"failed to patch MC-SMoE_math config copy: anchor not found in {path}")
...
def prepare(method: str, source: Path) -> Path:
    ...
    for src in sorted(source.iterdir()):
        dst = target / src.name
        if method == "MC-SMoE_math" and src.name == "config.json" and src.is_file():
            copy_text_file(src, dst)
            patch_mcsmoe_json_config_file(dst)
        elif method == "MC-SMoE_math" and src.suffix == ".py" and src.is_file():
            copy_text_file(src, dst)
