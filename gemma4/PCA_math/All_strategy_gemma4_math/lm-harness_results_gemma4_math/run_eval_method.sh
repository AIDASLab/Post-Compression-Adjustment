...
cd "${EXP_ROOT}"
ADDITIONAL_STRATEGY_ROOT="$(dirname "${EXP_ROOT}")"
...
Examples:
  bash run_eval_method.sh AIMER_math
  bash run_eval_method.sh AIMER_math --force
  bash run_eval_method.sh AIMER_math --categories code,AIME
...
Environment:
  ...
  USE_EXP_LOCAL_CACHE=1             Put HF/XDG caches under this lm-harness_results_gemma4_math/.cache.
  ...
  SKIP_MODEL_MIRROR=1               Evaluate original model directories directly.
...
method_dir_for() {
  case "$1" in
    AIMER_math) echo "${ADDITIONAL_STRATEGY_ROOT}/AIMER_math" ;;
    *) return 1 ;;
  esac
}
...
method_label_for() {
  case "$1" in
    AIMER_math) echo "AIMER_math_gemma4" ;;
    *) return 1 ;;
  esac
}
...
mkdir -p "${TORCHINDUCTOR_CACHE_DIR}" "${TRITON_CACHE_DIR}" "${VLLM_CACHE_ROOT}" "${HF_MODULES_CACHE}"
...

{
  ...
  echo "HF dynamic module cache is forced under this lm-harness_results_gemma4_math directory to avoid writing custom code cache outside it."
  echo "Torch/Triton/vLLM runtime caches are forced under this lm-harness_results_gemma4_math directory."
  echo "enable_thinking=False is forced in model_args for every category."
  echo "Model mirrors symlink large files and copy only patched config/python/processor files under lm-harness_results_gemma4_math/model_mirrors."
  ...
  echo "note=TMPDIR and VLLM_RPC_BASE_PATH intentionally use short relative paths under lm-harness_results_gemma4_math because Unix socket paths are limited."
