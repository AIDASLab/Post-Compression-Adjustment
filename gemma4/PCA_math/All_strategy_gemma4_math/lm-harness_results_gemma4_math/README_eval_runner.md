...
runner looks for `common/lm-evaluation-harness`; set `HARNESS_DIR` for another
location. The paper environment used lm-evaluation-harness version
`0.4.13.dev0`.
...
- `HF_MODULES_CACHE`, TorchInductor, Triton, vLLM, temp, and vLLM RPC paths are
  forced under this `lm-harness_results_gemma4_math` directory.
- Hugging Face model and dataset caches are not overridden by default. Set
  ...
  `lm-harness_results_gemma4_math/.cache`.

Useful options:

```bash
bash run_eval_method.sh AIMER_math --dry-run
bash run_eval_method.sh AIMER_math --categories code,AIME
HARNESS_DIR=/path/to/lm-evaluation-harness bash run_eval_method.sh AIMER_math --force
GPU_IDS=0,1 MAX_PARALLEL=2 bash run_eval_method.sh AIMER_math
```

Only AIMER is registered here because the Camera Ready math-domain robustness
experiment did not include Gemma4 M-SMoE.
