# Post-compression adjustment: Qwen3 + HC-SMoE + C4

This directory preserves the experiment layout used to adjust HC-SMoE-compressed
...
outputs under `qwen3/compression_qwen3/HC-SMoE`. They can be overridden with
`QWEN3_HCSMOE_RATIO_50`, `QWEN3_HCSMOE_RATIO_625`, and
...
For top-N adjustment, routed logical experts are counted on the calibration
split and mapped to HC-SMoE physical experts before selecting trainable expert
modules. The top-128 variants train every physical expert plus every router.
...
`VALIDATION_GPU` for the post-save validator. Hugging Face caches default to
the user's XDG cache location and may be changed with `HF_HOME` and
`HF_HUB_CACHE`.

Model checkpoints are produced under `ratio_*/<strategy>/` at runtime but are
not included in this repository. GPU telemetry covers expert selection when
applicable and the adjustment loop; it does not include model/data loading,
checkpoint saving, or post-save validation.
