...
def prune(
    ...
    for layer in tqdm(observer_data, "Pruning layers..."):
        ...
        experts_to_prune_list = sorted(
            int(i) for i in experts_to_prune.detach().cpu().tolist()
        )
        experts_to_prune_set = set(experts_to_prune_list)
        retained_expert_indicies = [
            i for i in range(num_experts) if i not in experts_to_prune_set
        ]
        layer_id = int(layer)
        retained_map[layer_id] = [int(i) for i in retained_expert_indicies]
        pruned_map[layer_id] = experts_to_prune_list
    ...
    with open(pruned_model_dir / "retained_expert_indices.json", "w", encoding="utf-8") as f:
        json.dump(retained_map, f, indent=2)
    with open(pruned_model_dir / "pruned_expert_indices.json", "w", encoding="utf-8") as f:
...
def get_pruned_model_dir(
    ...
    compression_ratio = n_experts_to_prune / total_experts
    compression_ratio_str = f"{compression_ratio:.6f}".rstrip("0").rstrip(".")
...
def main():
    ...
    local_files_only = _env_flag("REAP_LOCAL_FILES_ONLY", True)
    ...
    has_expert_index_maps = (
        (pruned_model_dir / "retained_expert_indices.json").is_file()
        and (pruned_model_dir / "pruned_expert_indices.json").is_file()
    )
    if (
        has_pruned_model
        and has_expert_index_maps
        ...
        if has_pruned_model and not has_expert_index_maps:
            logger.warning(
                f"Pruned model directory {pruned_model_dir} exists but is missing expert index maps. "
                "Re-running pruning so retained/pruned expert maps are saved."
            )
