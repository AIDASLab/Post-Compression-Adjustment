...
def _load_c4_dataset():
    raw_path = os.environ.get("C4_DATASET_PATH")
    if not raw_path:
        raise RuntimeError(
            "C4_DATASET_PATH must point to the uncompressed "
            "c4-train.00000-of-01024.json calibration shard."
        )
    calibration_data_path = Path(raw_path).expanduser().resolve()
    if not calibration_data_path.is_file():
        raise FileNotFoundError(f"C4 calibration file not found: {calibration_data_path}")
    return load_dataset(
        "json",
        data_files={"train": str(calibration_data_path)},
