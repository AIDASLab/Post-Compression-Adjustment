#!/usr/bin/env python3
"""Run one vLLM lm-evaluation-harness category for one original/compressed model."""
...
DEFAULT_HARNESS_DIR = Path(
    os.environ.get("HARNESS_DIR", REPO_ROOT / "common" / "lm-evaluation-harness")
).expanduser()
COMPRESSED_MODELS_ROOT = ROOT.parent
ALLOWED_LOCAL_MODEL_ROOTS = (
    ROOT / "model_mirrors",
...
def is_local_path_ref(model_ref: str) -> bool:
    return model_ref.startswith("/") or model_ref.startswith("./") or model_ref.startswith("../")


def normalize_model_ref(model_ref: str) -> str:
    if not is_local_path_ref(model_ref):
        return model_ref
    local_path = resolved_inside_any(Path(model_ref), ALLOWED_LOCAL_MODEL_ROOTS, "model ref")
    if not (local_path / "config.json").is_file():
        raise FileNotFoundError(f"model config not found: {local_path / 'config.json'}")
...
def build_model_args(
    ...
    if method == "HC-SMoE" and env_flag("HC_SMOE_ENFORCE_EAGER", True):
        args["enforce_eager"] = True
    if model_impl:
        args["model_impl"] = model_impl
...
def configure_code_eval_manager() -> None:
    if getattr(multiprocessing, "_origcomp_tcp_manager_patched", False):
        return
    ...
    multiprocessing._origcomp_tcp_manager_patched = True  # type: ignore[attr-defined]
    os.environ["ORIGCOMP_CODE_EVAL_MANAGER"] = "tcp_loopback"
...
def configure_local_runtime(category: str) -> None:
    ...
    # Python multiprocessing.Manager also uses AF_UNIX sockets; keep that path short.
    ...
    for name in ("TMPDIR", "TORCHINDUCTOR_CACHE_DIR", "TRITON_CACHE_DIR", "VLLM_CACHE_ROOT", "VLLM_RPC_BASE_PATH", "HF_MODULES_CACHE"):
...
def main() -> int:
    args = parse_args()
    output_dir = resolved_inside(args.output_dir, ROOT, "output dir")
    ...
    model_ref = normalize_model_ref(args.model_ref)
    ...
    print(f"method={args.method} ratio={args.ratio} variant={args.variant} category={args.category}")
    ...
    print(f"ORIGCOMP_CODE_EVAL_MANAGER={os.environ.get('ORIGCOMP_CODE_EVAL_MANAGER', '')}")
    print(f"VLLM_RPC_BASE_PATH={os.environ.get('VLLM_RPC_BASE_PATH', '')}")
    print(f"HC_SMOE_ENFORCE_EAGER={os.environ.get('HC_SMOE_ENFORCE_EAGER', '<default:1>')}")
