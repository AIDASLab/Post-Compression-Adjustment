#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Train/calibrate HC-SMoE checkpoints with comparable recovery strategies."""
...
def format_openr1_math_example(example: Dict) -> str:
    problem = _normalize_text(example.get("problem"))
    solution = _normalize_text(example.get("solution"))
    ...
    raise ValueError("Failed to construct calibration text from OpenR1 sample.")
...
class TextLineDataset(Dataset):
    def __init__(self, path: str, max_samples: Optional[int] = None):
        ...
        if path == OPENR1_MATH_DATASET:
            self._load_openr1_math(max_samples=max_samples)
        else:
            self._load_local_file(path)

        if path != OPENR1_MATH_DATASET and max_samples is not None:
    ...
    def _load_openr1_math(self, max_samples: Optional[int]) -> None:
        try:
            from datasets import load_dataset
        except ImportError as exc:
            raise ImportError("Loading OpenR1-Math-220k requires the `datasets` package.") from exc
        ...
        row_count = total_rows if max_samples is None else min(max_samples, total_rows)
        hf_dataset = hf_dataset.select(range(row_count))

        for row_idx, example in enumerate(hf_dataset):
            try:
                text = format_openr1_math_example(example)
            except ValueError as exc:
                raise ValueError(f"Failed to construct calibration text from OpenR1 row {row_idx}.") from exc
            if text:
                self.samples.append(text)

        print(
            f"[Dataset] Source={OPENR1_MATH_DATASET}:{OPENR1_MATH_CONFIG}/{OPENR1_MATH_SPLIT} "
            f"first_rows={row_count} total_rows={total_rows}",
            flush=True,
        )
...
def load_external_expert_mapping(checkpoint_path: str) -> Dict[int, List[int]]:
    mapping_path = Path(checkpoint_path) / "expert_mapping.json"
    ...
    for layer_key, layer_payload in payload.get("layers", {}).items():
        ...
        for group in groups:
            physical_id = int(group["physical_expert_id"])
            for original_id in group.get("original_expert_ids", []):
                pairs.append((int(original_id), physical_id))
        if not pairs:
            continue
        max_original = max(original_id for original_id, _ in pairs)
        mapping = [-1] * (max_original + 1)
        for original_id, physical_id in pairs:
            mapping[original_id] = physical_id
        if any(value < 0 for value in mapping):
            missing = [idx for idx, value in enumerate(mapping) if value < 0][:10]
            raise RuntimeError(
                f"expert_mapping.json layer {layer_key} does not cover every logical expert. "
                f"Missing examples: {missing}"
            )
        output[int(layer_key)] = mapping
    return output
...
def save_router_logit_mapping_audit(
    ...
    external_mapping = load_external_expert_mapping(source_checkpoint_path(args))
    if not external_mapping:
        raise FileNotFoundError(
            f"DRLM requires HC-SMoE expert_mapping.json under {source_checkpoint_path(args)}"
        )
    ...
    for prefix in student_prefixes:
        ...
        module_mapping = get_logical_to_physical(module, torch.device("cpu")).cpu().tolist()
        external_layer_mapping = external_mapping.get(layer_idx)
        layers.append(
            {
                "layer_index": layer_idx,
                "moe_prefix": prefix,
                "logical_num_experts": len(module_mapping),
                "physical_num_experts": int(max(module_mapping) + 1) if module_mapping else 0,
                "module_logical_to_physical": module_mapping,
                "external_mapping_present": external_layer_mapping is not None,
                "external_mapping_matches_module": (
                    external_layer_mapping == module_mapping if external_layer_mapping is not None else None
                ),
            }
        )
    ...
    mismatched_layers = [
        layer["layer_index"]
        for layer in layers
        if layer["external_mapping_matches_module"] is not True
    ]
    audit = {
        "ratio": args.ratio,
        "strategy": args.strategy,
        "student_checkpoint": source_checkpoint_path(args),
        "expert_mapping_json": str(Path(source_checkpoint_path(args)) / "expert_mapping.json"),
        "num_layers": len(layers),
        "all_external_mappings_match_module": not mismatched_layers,
    ...
    if mismatched_layers:
        raise RuntimeError(
            "HC-SMoE expert mapping mismatch between expert_mapping.json and model modules. "
            f"Mismatched layer examples: {mismatched_layers[:10]}"
        )


def aggregate_logits_by_mapping(
    logits: torch.Tensor,
    logical_to_physical: torch.Tensor,
    output_dim: int,
) -> torch.Tensor:
    original_shape = tuple(logits.shape[:-1])
    input_dim = int(logits.shape[-1])
    mapping = logical_to_physical.to(device=logits.device, dtype=torch.long)
    if mapping.numel() != input_dim:
        raise ValueError(
            f"Cannot aggregate router logits: mapping length {mapping.numel()} != logits dim {input_dim}."
        )
    if mapping.numel() == 0:
        raise ValueError("Cannot aggregate router logits with an empty mapping.")
    if int(mapping.min().item()) < 0 or int(mapping.max().item()) >= output_dim:
        raise ValueError(
            f"Mapping values must stay in [0, {output_dim}); observed "
            f"min={int(mapping.min().item())}, max={int(mapping.max().item())}."
        )

    flat_logits = logits.reshape(-1, input_dim).float()
    ...
    aggregated.index_add_(1, mapping, flat_logits)
    counts = torch.zeros(output_dim, dtype=torch.float32, device=flat_logits.device)
    counts.index_add_(0, mapping, torch.ones_like(mapping, dtype=torch.float32))
    aggregated = aggregated / counts.clamp_min(1.0).unsqueeze(0)
    return aggregated.reshape(*original_shape, output_dim)
...
def align_teacher_router_logits_for_student(
    ...
    mapping = logical_to_physical.to(device=teacher_layer_logits.device, dtype=torch.long)
    if teacher_dim == mapping.numel() and student_dim <= int(mapping.max().item()) + 1:
        aligned = aggregate_logits_by_mapping(
            teacher_layer_logits,
            logical_to_physical=mapping,
            output_dim=student_dim,
        )
        return aligned.to(device=student_layer_logits.device)

    if student_dim == mapping.numel() and teacher_dim > int(mapping.max().item()):
        aligned = torch.index_select(teacher_layer_logits, dim=-1, index=mapping)
        return aligned.to(device=student_layer_logits.device)

    raise ValueError(
        f"Router logit dimension mismatch at layer {layer_idx}: "
        f"teacher_dim={teacher_dim}, student_dim={student_dim}, mapping_len={mapping.numel()}, "
        f"mapping_max={int(mapping.max().item()) if mapping.numel() else 'n/a'}"
    )
...
def router_logit_matching_loss(
    ...
    student_module_dict: Dict[str, nn.Module],
    temperature: float,
) -> torch.Tensor:
    ...
    for layer_idx, (teacher_layer_logits, student_layer_logits) in enumerate(
        ...
        logical_to_physical = get_logical_to_physical(
            student_module_dict[prefix],
            torch.device("cpu"),
        )
        ...
        if tuple(aligned_teacher_logits.shape) != tuple(student_layer_logits.shape):
            raise ValueError(
                f"Aligned router logit shape mismatch at layer {layer_idx}: "
        ...
        if tokenwise_mse.numel() != flat_mask.numel():
            raise ValueError(
                f"Router token count mismatch at layer {layer_idx}: "
...
def save_hcsmoe_sidecars(source_dir: str, output_dir: str) -> None:
...
def save_model(
    ...
    save_hcsmoe_sidecars(source_checkpoint_path(args), args.output_dir)
...
def train_direct_router_logit_matching(
    ...
    save_router_logit_mapping_audit(args, student, student_prefixes)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HC-SMoE recovery strategy trainer.")
    ...
    parser.add_argument("--calib_dataset_path", required=True, help="Local .txt/.json path or open-r1/OpenR1-Math-220k.")
    ...
    parser.add_argument("--max_calib_samples", type=int, default=1024)
