#!/usr/bin/env python3
"""Run one vLLM lm-evaluation-harness category for one local model."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one model/category with lm-evaluation-harness vLLM."
    )
...
def resolved_inside_any(path: Path, roots: tuple[Path, ...], label: str) -> Path:
    resolved = path.resolve()
    root_resolved = tuple(root.resolve() for root in roots)
    if not any(resolved.is_relative_to(root) for root in root_resolved):
        roots_text = ", ".join(str(root) for root in root_resolved)
...
def build_model_args(
    ...
    if method == "HC-SMoE_math" and env_flag("HC_SMOE_ENFORCE_EAGER", True):
...
def configure_code_eval_manager() -> None:
    if getattr(multiprocessing, "_qwen3_strategy_tcp_manager_patched", False):
        return
    ...
    multiprocessing._qwen3_strategy_tcp_manager_patched = True  # type: ignore[attr-defined]
    os.environ["QWEN3_STRATEGY_CODE_EVAL_MANAGER"] = "tcp_loopback"
...
def configure_local_runtime(category: str) -> None:
    ...
    os.environ.setdefault("TORCH_EXTENSIONS_DIR", str(EXP_ROOT / ".cache" / "torch_extensions"))
    ...
    os.environ.setdefault("HF_MODULES_CACHE", str(EXP_ROOT / ".cache" / "huggingface" / "modules"))
    for name in (
        "TMPDIR",
...
def main() -> int:
    ...
    method_root = MODEL_ROOT / args.method
    mirror_method_root = EXP_ROOT / "model_mirrors" / args.method
    model_path = resolved_inside_any(
        args.model_path, (method_root, mirror_method_root), "model path"
    )
    output_dir = resolved_inside(args.output_dir, EXP_ROOT, "output dir")
    harness_dir = resolved_inside(args.harness_dir, DEFAULT_HARNESS_DIR, "harness dir")
    ...
    print(f"QWEN3_STRATEGY_CODE_EVAL_MANAGER={os.environ.get('QWEN3_STRATEGY_CODE_EVAL_MANAGER', '')}")
