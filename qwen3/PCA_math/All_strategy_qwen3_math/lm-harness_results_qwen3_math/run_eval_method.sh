...
Examples:
  ...
  bash run_eval_method.sh HC-SMoE_math --dry-run
...
Environment:
  ...
  USE_EXP_LOCAL_CACHE=1             Put HF/XDG caches under lm-harness_results_qwen3_math/.cache.
  ...
  HC_SMOE_ENFORCE_EAGER=0           Disable HC-SMoE_math eager-mode fallback if needed.
...
CACHE_LOG="${LOG_ROOT}/cache_policy.log"
{
  ...
  echo "HF dynamic module cache is forced under lm-harness_results_qwen3_math to avoid writing custom code cache outside the work dir."
  echo "vLLM RPC, TMPDIR, torchinductor, Triton, and torch extension caches are forced under lm-harness_results_qwen3_math."
